/mob/Monsters/Egg //Probably should make this a sub-type of critter..
	...
	Age=0
	DieAge=999
	var/tmp //Reason it's temp is so as to evade saving duplicates of particular units and to not have breeding details carry in saves.
...
/mob/proc/BreedWith(mob/TARGET,ALLOWED)
	...
	if(Gender=="Male" && TARGET.Gender=="Female")
		MALE=src
		FEMALE=TARGET
	else if(TARGET.Gender=="Male" && Gender=="Female")
		MALE=TARGET
		FEMALE=src
	switch(MALE.Race)
