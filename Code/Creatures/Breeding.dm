/mob/Monsters/Egg //Probably should make this a sub-type of critter..
	...
	var/mob/CHILDTYPE //This holds the actual type value that the child will be. In most cases equal to FEMALE's racetype durring births, but needs to be set for eggs.
	var/mob/CHILDCOUNT //This holds the ammount of children that will be created upon "GiveBirth()".

/mob/proc/BreedWith(var/mob/target, var/allowed)
			...
			if(allowed)
				...
				switch(female.Race)
					if("Spider")
						if(female.CanBreed == 0 && female.SubRace=="Queen")
							if(Age>=29) //Should really use a diffrent var.. maybe just go on icon? Deal with it when I finish breeding off.
								...
								female.icon = 'icons/mobs/SpiderQueenMature.dmi'
					...
					if("Lizardman")
						...
						female.Breeding(male,female) //female is both src, and an argument in traditional cases.

/mob/proc/Breeding(var/mob/male, var/mob/female, TIME=3000) //src is the one having the kid, the arguments are just for setting statistics.
	...
	female.CHILDMALE=male //Durring birth this will be set to CHILDFEMALE if null.
	female.CHILDFEMALE=female //Durring birth this will be set to CHILDMALE if null.
	female.CHILDCOUNT=1 //Number to create upon giving birth()
	female.CHILDTYPE=text2path("/mob/Monsters/[female.Race]")
	if(female.Critter)
	...
	if(WasSet)
		female=null //As a result we follow the old method of override.
